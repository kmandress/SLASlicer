...
public:
	...
	void BoundingBox(Point3D& BBMin, Point3D& BBMax)
	{
		// Initialize BBox
		for (int i = 0; i < 3; i++)
		{
			BBMin[i] = facets[0].BBMin()[i];
			BBMax[i] = facets[0].BBMax()[i];
		}

		for (auto facet : facets)
		{
			auto thisMin = facet.BBMin();
			auto thisMax = facet.BBMax();

			for (int i = 0; i < 3; i++)
			{
				BBMin[0] = min(BBMin[0], thisMin[0]);
				BBMin[1] = min(BBMin[1], thisMin[1]);
				BBMin[2] = min(BBMin[2], thisMin[2]);

				BBMax[0] = max(BBMax[0], thisMax[0]);
				BBMax[1] = max(BBMax[1], thisMax[1]);
				BBMax[2] = max(BBMax[2], thisMax[2]);
			}
		}
	}
	...
	{
		for (auto &facet : facets)
			facet.Translate(xShift, yShift, zShift);
	}
